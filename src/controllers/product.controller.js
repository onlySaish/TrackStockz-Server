...
const createProduct = asyncHandler(async (req, res) => {
    ...
    const {name,description,lowStockThreshold,category,price,discountPercent,quantity,supplier} = req.body;
    if (!(name && description && lowStockThreshold && category && price)) {throw new ApiError("Missing Required Fields")};
...
const getAllProducts = asyncHandler(async (req, res) => {
    try {
        ...
        let { page = 1, limit = 5, sort = "createdAt", order = "asc", search = "", isDeleted = "false", category = "", status = "" } = req.query;
        ...
        // Default filter
        let matchConditions = [{ isDeleted: deletedFlag, owner: userId }];
