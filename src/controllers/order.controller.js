...
const addOrder = asyncHandler(async (req, res) => {
  ...
  // Verify membership to the customer's organization
  const membership = await Membership.findOne({ user: userId, organization: customer.organization, status: "Active" });
  if (!membership) {
    throw new ApiError(403, "You do not have access to this customer's organization");
  }
  ...
  for (const item of products) {
    ...
    // Check if product belongs to the same organization
    if (product.organization.toString() !== customer.organization.toString()) {
      throw new ApiError(400, `Product ${product.name} does not belong to the same organization`);
    }
...
const getAllOrders = asyncHandler(async (req, res) => {
  try {
    ...
    let { page = 1, limit = 5, sort = "createdAt", order = "asc", search = "", status = "", paymentMethod = "", organizationId } = req.query;
...
const editOrder = asyncHandler(async (req, res) => {
  ...
  // Verify membership
  const membership = await Membership.findOne({ user: req.user._id, organization: order.organization, status: "Active" });
  if (!membership) {
...
const updateStatus = asyncHandler(async (req, res) => {
  ...
  // Verify membership
  const membership = await Membership.findOne({ user: req.user._id, organization: order.organization, status: "Active" });
  if (!membership) throw new ApiError(403, "You do not have access to this order");
