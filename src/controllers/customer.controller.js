...
const addCustomer = asyncHandler(async (req, res) => {
    ...
    const { firstName, lastName, email, phoneNumber, address, companyName, organizationId } = req.body;

    if (!(email && firstName && phoneNumber && organizationId)) { throw new ApiError(404, "All Fields are Required") };
    ...
    if (existingCustomer) { throw new ApiError(400, "Customer Already Exists in this Organization") };
...
const updateCustomerDetails = asyncHandler(async (req, res) => {
    ...
    const membership = await Membership.findOne({ user: req.user._id, organization: existingCustomer.organization, status: "Active" });
    if (!membership) {
...
const toggleBlacklistCustomer = asyncHandler(async (req, res) => {
    ...
    // Verify membership
    const membership = await Membership.findOne({ user: req.user._id, organization: customer.organization, status: "Active" });
    if (!membership) {
        throw new ApiError(403, "You do not have access to this customer");
    }
...
const getAllCustomers = asyncHandler(async (req, res) => {
    try {
        const userId = req.user._id;
        let { page = 1, limit = 5, sort = "createdAt", order = "asc", search = "", blacklist = "false", organizationId } = req.query;
        ...
        let matchStage = { $match: { blackListed: isBlackListed, organization: new mongoose.Types.ObjectId(organizationId) } };
